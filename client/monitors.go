...
func (c *Client) GetAllMonitors() (*[]Monitor, error) {
	body, err := c.httpRequest("/monitor/all", "GET", bytes.Buffer{})
	if nil != err {
		return nil, err
	}
	response := []Monitor{}
...
func (c *Client) GetMonitorById(monitor_id string) (*Monitor, error) {
	body, err := c.httpRequest(fmt.Sprintf("/monitor/%s", monitor_id), "GET", bytes.Buffer{})
	if nil != err {
		return nil, err
	}
	monitor := &Monitor{}
...
func (c *Client) AddMonitor(monitor Monitor) (*Monitor, error) {
	...
	resp_body, err := c.httpRequest("/monitor", "POST", buf)
	if nil != err {
		return nil, err
	}
...
func (c *Client) UpdateMonitorById(monitorId string, monitor Monitor) (*Monitor, error) {
	...
	resp_body, err := c.httpRequest(fmt.Sprintf("/monitor/%s", monitorId), "PUT", buf)
	if nil != err {
		return nil, err
	}
...
func (c *Client) DeleteMonitorById(monitorId string) error {
	_, err := c.httpRequest(fmt.Sprintf("/monitor/%s", monitorId), "DELETE", bytes.Buffer{})
	if nil != err {
		return err
	}
	return nil
